...
class Adminclientagencies extends CI_Controller {
    ...
    function index() {
        $data['title'] = "RADgov - Client Agencies";
        $data["page"] = "client_agencies";
        $data["sidebar"] = $this->load->view("admin/sidebar_view", $data, TRUE);
        $this->load->view('admin/header_view', $data);
        $this->load->view('admin/client_agency_view', $data);
        $this->load->view('admin/footer_view', $data);
    }

    function add_edit_client_agency() {
        $data['id'] = $this->input->get_post("id");
        $str = $this->load->view("admin/add_edit_client_agency", $data, true);
        $value = array(
            "str" => $str
        );
        echo json_encode($value);
    ...
    function save() {
        ...
        $result = $this->adminmodel->save_client_agency($data);
        $value = array(
            "result" => $result
        );
        echo json_encode($value);
    ...
    function check_client_agency_exist() {
        $name = strtolower($this->admin->escape_special_chrs(trim($this->input->get_post("agency_name"))));
        $id = $this->admin->escape_special_chrs(trim($this->input->get_post("id")));
        $query = $this->adminmodel->check_client_agency_availability($name, $id);
        $q = $query->num_rows();
        $result = "true";
        if ($q > 0) {
            $result = "false";
    ...
    function del_client_agency() {
        $id = $this->input->get_post("id");
        $result = $this->adminmodel->del_client_agency($id);
        $value = array(
            "result" => $result
        );
        echo json_encode($value);
