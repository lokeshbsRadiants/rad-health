...
class Admincontractvehicles extends CI_Controller {
    ...
    function index() {
        $data['title'] = "RADgov - Contract Vehicles";
        $data["page"] = "contract_vehicles";
        $data["sidebar"] = $this->load->view("admin/sidebar_view", $data, TRUE);
        $this->load->view('admin/header_view', $data);
        $this->load->view('admin/contract_vehicle_view', $data);
        $this->load->view('admin/footer_view', $data);
    }

    function add_edit_contract_vehicle() {
        $data['id'] = $this->input->get_post("id");
        $str = $this->load->view("admin/add_edit_contract_vehicle", $data, true);
        $value = array(
            "str" => $str
        );
        echo json_encode($value);
    ...
    function save() {
        ...
        $result = $this->adminmodel->save_contract_vehicle($data);
        $value = array(
            "result" => $result
        );
        echo json_encode($value);
    ...
    function check_contract_vehicle_exist() {
        $name = strtolower($this->admin->escape_special_chrs(trim($this->input->get_post("contract_name"))));
        $id = $this->admin->escape_special_chrs(trim($this->input->get_post("id")));
        $query = $this->adminmodel->check_contract_vehicle_availability($name, $id);
        $q = $query->num_rows();
        $result = "true";
        if ($q > 0) {
            $result = "false";
    ...
    function del_contract_vehicle() {
        $id = $this->input->get_post("id");
        $result = $this->adminmodel->del_contract_vehicle($id);
        $value = array(
            "result" => $result
        );
        echo json_encode($value);
    ...
    function save_image() {
        $dir = realpath("assets/uploads/vehicles/");
    ...
    function delete_image() {
        unlink(realpath("assets/uploads/vehicles") . "/" . $_POST['file']);
        $value = array(
            "result" => "Success"
        );
        echo json_encode($value);
