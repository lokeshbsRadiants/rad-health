...
class Adminservicedetail extends CI_Controller {
    ...
    function index() {
        $data['title'] = "RADgov - Service Detail";
    ...
    function add_edit_service_detail() {
        $data['id'] = $this->input->get_post("id");
        $str = $this->load->view("admin/add_edit_service_detail", $data, true);
        $value = array(
            "str" => $str
        );
        echo json_encode($value);
    ...
    function save() {
        ...
        unset($data['total_features']);
        if (isset($data['key_features'])) {
            $data['key_features'] = implode('~', $data['key_features']);
        }
        if (isset($data['key_icons'])) {
            $data['key_icons'] = implode('~', $data['key_icons']);
        }
        $data['modified_date'] = $this->admin->get_custom_date("%Y-%m-%d %H:%i:%s", now());
        $result = $this->adminmodel->save_service_detail($data);
        $value = array(
            "result" => $result
        );
        echo json_encode($value);
    ...
    function check_service_detail_exist() {
        $name = strtolower($this->admin->escape_special_chrs(trim($this->input->get_post("service_detail_name"))));
        $id = $this->admin->escape_special_chrs(trim($this->input->get_post("id")));
        $query = $this->adminmodel->check_service_detail_availability($name, $id);
        $q = $query->num_rows();
        $result = "true";
        if ($q > 0) {
            $result = "false";
    ...
    function del_service_detail() {
        ...
        $result = $this->adminmodel->del_service_detail($id);
        $value = array(
            "result" => $result
        );
        echo json_encode($value);
    ...
    function save_image() {
        $dir = realpath("assets/uploads/services/");
        $index = isset($_FILES['files1']['name']) ? 1 : (isset($_FILES['files2']['name']) ? 2 : (isset($_FILES['files3']['name']) ? 3 : (isset($_FILES['files4']['name']) ? 4 : 5)));
    ...
    function delete_image() {
        unlink(realpath("assets/uploads/services") . "/" . $_POST['file']);
        $value = array(
            "result" => "Success"
        );
        echo json_encode($value);
