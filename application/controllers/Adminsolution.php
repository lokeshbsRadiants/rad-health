...
class Adminsolution extends CI_Controller {
    ...
    function index() {
        $data['title'] = "RADgov - Solutions";
        $data["page"] = "solution";
        $data["sidebar"] = $this->load->view("admin/sidebar_view", $data, TRUE);
        $this->load->view('admin/header_view', $data);
        $this->load->view('admin/solution_view', $data);
        $this->load->view('admin/footer_view', $data);
    }

    function add_edit_solution() {
        $data['id'] = $this->input->get_post("id");
        $str = $this->load->view("admin/add_edit_solution", $data, true);
        $value = array(
            "str" => $str
        );
        echo json_encode($value);
    ...
    function save() {
        ...
        $result = $this->adminmodel->save_solution($data);
        $value = array(
            "result" => $result
        );
        echo json_encode($value);
    ...
    function check_solution_exist() {
        ...
        $query = $this->adminmodel->check_solution_availability($name, $id);
        $q = $query->num_rows();
        $result = "true";
        if ($q > 0) {
            $result = "false";
    ...
    function del_solution() {
        $id = $this->input->get_post("id");
        $result = $this->adminmodel->del_solution($id);
        $value = array(
            "result" => $result
        );
        echo json_encode($value);
    ...
    function save_image() {
        $dir = realpath("assets/uploads/solution/");
    ...
    function delete_image() {
        unlink(realpath("assets/uploads/solution") . "/" . $_POST['file']);
        $value = array(
            "result" => "Success"
        );
        echo json_encode($value);
