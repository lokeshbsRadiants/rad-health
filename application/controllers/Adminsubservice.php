...
class Adminsubservice extends CI_Controller {
    ...
    function index() {
        $data['title'] = "RADgov - Sub Services";
        $data["page"] = "sub_service";
        $data["sidebar"] = $this->load->view("admin/sidebar_view", $data, TRUE);
        $this->load->view('admin/header_view', $data);
        $this->load->view('admin/sub_service_view', $data);
        $this->load->view('admin/footer_view', $data);
    }

    function add_edit_sub_service() {
        $data['id'] = $this->input->get_post("id");
        $str = $this->load->view("admin/add_edit_sub_service", $data, true);
        $value = array(
            "str" => $str
        );
        echo json_encode($value);
    ...
    function save() {
        ...
        $result = $this->adminmodel->save_sub_service($data);
        $value = array(
            "result" => $result
        );
        echo json_encode($value);
    ...
    function check_sub_service_exist() {
        $name = strtolower($this->admin->escape_special_chrs(trim($this->input->get_post("sub_service_name"))));
        $id = $this->admin->escape_special_chrs(trim($this->input->get_post("id")));
        $query = $this->adminmodel->check_sub_service_availability($name, $id);
        $q = $query->num_rows();
        $result = "true";
        if ($q > 0) {
            $result = "false";
    ...
    function del_sub_service() {
        $id = $this->input->get_post("id");
        $result = $this->adminmodel->del_sub_service($id);
        $value = array(
            "result" => $result
        );
        echo json_encode($value);
